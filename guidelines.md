...
## 5. Current State (v1.3 - MIDI Velocity & Calibration)
...
- **Интеграция стандартов MIDI:** Параметры Velocity (`velo`) и Randomization (`human`) работают в диапазоне от 0 до 127.
- **Калибровка оборудования:** Добавлен параметр `base` (0-255), задающий минимальное напряжение страгивания соленоида (Floor PWM). В режиме редактирования `base` соленоид лупит без рандомизации для точной настройки порога.
- При `velo = 0` на выход подается аппаратный `LOW` (тишина). При `velo = 127` — аппаратный `HIGH` (игнорируя ШИМ для максимальной мощности).
- **Сброс настроек UI:** Долгое удержание энкодера в меню сбрасывает текущий выбранный параметр на значение по умолчанию.
- Мгновенный отклик UI (нулевая задержка кнопок при выборе канала через `.press()`).
- Кастомная попиксельная отрисовка списков меню с динамической инверсией выделения.

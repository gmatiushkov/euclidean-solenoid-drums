...
## 5. Current State (v1.4 - Dual MIDI & MIDI Learn)
...
- **Интеграция стандартов MIDI Velocity:** Сила удара по MIDI привязана к ШИМ: `velo = 0` (Note Off) выдает LOW, `1-126` масштабируется с учетом параметра калибровки `base`, `velo = 127` выдает жесткий HIGH для максимальной мощности.
...
- **Память (EEPROM)**: При расширении количества каналов следует учитывать размер структуры `SaveData` и пропорционально увеличивать выделяемый буфер в `EEPROM.begin()`. Магическое число обновляется при каждом изменении структуры.